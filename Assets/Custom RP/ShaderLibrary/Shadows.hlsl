...
ShadowData GetShadowData(Surface surface)
{
    ...
    for (i = 0; i < _CascadeCount; ++i)
    {
        ...
        {
            const float fade = FadeShadowStrength(distanceSqr, _CascadeData[i].x,  _ShadowDistanceFade.z);
...
float GetDirectionalShadowAttenuation(DirectionalShadowData directional, ShadowData global, Surface surface)
{
    ...
    float3 normalBias = surface.normal * directional.normalBias
        * _CascadeData[global.cascadeIndex].y;
    
    float3 positionSTS = mul(
        _DirectionalShadowMatrices[directional.tileIndex],
        float4(surface.position + normalBias, 1.0f)
    ).xyz;
    ...
    {
        normalBias = surface.normal * directional.normalBias
